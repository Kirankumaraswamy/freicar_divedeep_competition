...
class FreiCarLoader(data.Dataset):
    ...
    def __init__(self, data_dir, padding, split='training', load_real=False):
        ...
        if split == 'training':
            ...
            print('eval loaded', len(self.rgb_path_files))
        else:
    ...
    def __getitem__(self, index):
        ...
        # TODO: Implement me!
        try:
            out['rgb'] = loadRGB(paths=[syn_rgb_path], size=None, pad_info=self.padding)[0]
            out['seg'] = loadRegressionOrSegmentation(paths=[seg_path], size=None, pad_info=self.padding)[0]
            out['reg'] = loadRegressionOrSegmentation(paths=[reg_path], size=None, pad_info=self.padding)[0]
