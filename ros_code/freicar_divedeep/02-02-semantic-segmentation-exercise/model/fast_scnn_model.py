...
class Fast_SCNN(torch.nn.Module):
    def __init__(self, input_channel, num_classes):
        ...
        self.classifier = ClassifierSeg(num_classes)
        #Adding lane classifier
        self.laneclassifier = ClassifierReg(1)

    def forward(self, x):
        shared = self.learning_to_downsample(x)
        x = self.global_feature_extractor(shared)
        x = self.feature_fusion(shared, x)
        l = self.laneclassifier(x)
        x = self.classifier(x)
        x = torch.nn.functional.interpolate(x, scale_factor=8, mode='bilinear')
        l = torch.nn.functional.interpolate(l, scale_factor=8, mode='bilinear')
        return x, l
