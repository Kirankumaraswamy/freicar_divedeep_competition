...
void PurePursuit::controller_step(nav_msgs::Odometry odom)
{
    ...
    double car_angle = atan2((odom.pose.pose.position.y-ra_t_map.getOrigin().y()), (odom.pose.pose.position.x-ra_t_map.getOrigin().x())) * (180/3.142);
    ...
    if(path_.size() > 0 && !goal_reached_){
        ...
        for( int i=1; i < path_.size(); i++) {
            ...
            double odom_dist1 = sqrt(pow(path_.at(i-1).getOrigin().x() - odom.pose.pose.position.x, 2) + pow(path_.at(i-1).getOrigin().y() - odom.pose.pose.position.y, 2));
            double odom_dist2 = sqrt(pow(path_.at(i).getOrigin().x() - odom.pose.pose.position.x, 2) + pow(path_.at(i).getOrigin().y() - odom.pose.pose.position.y, 2));
            ...
            if(minimum_dist>(odom_dist1+odom_dist2)/2){
                ...
                //slope in map frame
                double map_m = (path_.at(i).getOrigin().y()-path_.at(i-1).getOrigin().y())/(path_.at(i).getOrigin().x()-path_.at(i-1).getOrigin().x());
                //Logic to find cross point error.
                double A, B, C;
                A = map_m; B = -1; C = path_.at(i-1).getOrigin().y() - map_m * path_.at(i-1).getOrigin().x();
                //perpendicular distance between the point(middle of car axis) and the line(formed by 2 path points)
                cross_track_error_dist = abs(A*odom.pose.pose.position.x + B*odom.pose.pose.position.y + C)/sqrt(A*A + B*B);
