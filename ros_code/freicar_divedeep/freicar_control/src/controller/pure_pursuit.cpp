 ...
 * the pub_acker_ publisher.
 */
    void PurePursuit::controller_step1(geometry_msgs::PoseArray msg)
    {
        // Code blocks that could be useful:
        // The following code block could receive the current pose (saved in map_t_ra)
        geometry_msgs::TransformStamped tf_msg, ra_t_map_msg;
        ...
        if(path_.size() > 0 && !goal_reached_){
            ...
            for( int i=1; i < path_.size(); i++) {
                ...
                //These distance are used to cross track error
                double odom_dist1 = sqrt(pow(path_.at(i-1).getOrigin().x() - msg.poses.data()->position.x, 2) + pow(path_.at(i-1).getOrigin().y() - msg.poses.data()->position.y, 2));
                double odom_dist2 = sqrt(pow(path_.at(i).getOrigin().x() - msg.poses.data()->position.x, 2) + pow(path_.at(i).getOrigin().y() - msg.poses.data()->position.y, 2));
                ...
                if(minimum_dist>(odom_dist1+odom_dist2)/2){
                    ...
                    //perpendicular distance between the point(middle of car axis) and the line(formed by 2 path points)
                    cross_track_error_dist = abs(A*msg.poses.data()->position.x + B*msg.poses.data()->position.y + C)/sqrt(A*A + B*B);
                    //std::cout<<"index : " << i <<" d: " << minimum_dist << " error: " << cross_track_error_dist<<std::endl;
                    path_angle = atan(map_m) * (180/3.142);
                    //std::cout<<" index:" << i << std::endl;
                }
